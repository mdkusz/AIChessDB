...
{
    /// <summary>
    /// SHows a dialog box to select a file name
    /// </summary>
    public class FileNameInputEditor : DIalogBoxInputEditor
    {
        private Label _fileLabel;
        public FileNameInputEditor(PropertyEditorInfo pinfo, object instance, Control container) : base(pinfo, instance, container)
        {
            if (pinfo.EditorType != InputEditorType.FileName)
            {
        ...
        protected override void ShowDialog(object sender, EventArgs e)
        {
            ...
            {
                _property.SetValue(_instance, cd.FileName);
                _fileLabel.Text = Path.GetFileName(cd.FileName);
            }
        }
    }
}
