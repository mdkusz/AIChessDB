...
{
    /// <summary>
    /// Editor to edit integer values with a NumericUpDown control
    /// </summary>
    /// <remarks>
    ...
    /// When EditorType is not IntValue, the property is not found, or the property is not an integer value
    /// </exception>
    ...
    public class IntValueInputEditor : PropertyInputEditorBase
    {
        private CheckBox _nullCB = null;
        public IntValueInputEditor(PropertyEditorInfo pinfo, object instance, Control container) : base(pinfo, instance, container)
        {
            if (pinfo.EditorType != InputEditorType.IntValue)
            {
            ...
            if (!new List<Type> { typeof(sbyte), typeof(byte), typeof(short), typeof(ushort), typeof(int),
                typeof(uint), typeof(long), typeof(ulong),
                typeof(sbyte?), typeof(byte?), typeof(short?), typeof(ushort?), typeof(int?),
                typeof(uint?), typeof(long?), typeof(ulong?)}.Contains(_property.PropertyType))
            {
        ...
        protected override void AddControl(Control container, string text = null)
        {
            ...
            {
                nup.Minimum = Convert.ToInt32(_pInfo.Values[0]);
                nup.Maximum = Convert.ToInt32(_pInfo.Values[1]);
                nup.Increment = Math.Max((nup.Maximum - nup.Minimum) / 10M, 1M);
            }
            ...
            {
                nup.Width = (int)Math.Ceiling(g.MeasureString(nup.Value.ToString(), container.Font).Width) + SystemInformation.VerticalScrollBarWidth + 8;
            }
            ...
            using (Graphics gr = control.CreateGraphics())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        int maxw = (int)Math.Ceiling(gr.MeasureString(nup.Value.ToString() + SystemInformation.VerticalScrollBarWidth + 8, control.Font).Width);
